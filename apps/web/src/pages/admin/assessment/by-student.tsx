...
const AssessmentByStudent = () => {
  const { openMobileNav } = useOutletContext<{ openMobileNav: () => void }>();
  const [classrooms, setClassrooms] = useState<ClassroomInfo[]>([]);
  const [selectedClassroomId, setSelectedClassroomId] = useState<string>("");
  const [allStudents, setAllStudents] = useState<ModuleStudent[]>([]);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-5 sm:py-8 space-y-5 sm:space-y-6">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200/60 p-4 sm:p-6">
         <div className="flex items-start gap-3">
                    ...
                    />
                    <div>
                        <h1 className="text-lg sm:text-2xl font-bold text-[#292382]">Assessment by Student</h1>
          <p className="text-xs sm:text-sm text-gray-500 mt-1">Select a class to view and search students</p>
                    </div>
                  </div>
...
export default AssessmentByStudent;
